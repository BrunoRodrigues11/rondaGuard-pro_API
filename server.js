...
const pool = mysql.createPool({
  ...
  database: process.env.DB_NAME || 'rondaguard_pro',
...
app.post('/api/templates', async (req, res) => {
  ...
  try {
    ...
    // Upsert Template
    const [existing] = await connection.query('SELECT id FROM checklist_templates WHERE id = ?', [id]);
    if (existing.length > 0) {
      await connection.query('UPDATE checklist_templates SET name = ? WHERE id = ?', [name, id]);
      await connection.query('DELETE FROM checklist_template_items WHERE template_id = ?', [id]);
    } else {
      await connection.query('INSERT INTO checklist_templates (id, name) VALUES (?, ?)', [id, name]);
    }
...
app.get('/api/tasks', async (req, res) => {
  try {
    ...
    for (const t of tasks) {
      const [items] = await pool.query('SELECT id, label, is_checked FROM task_checklist_items WHERE task_id = ?', [t.id]);
      result.push({
        id: t.id,
        title: t.title,
        sector: t.sector,
...
app.post('/api/tasks', async (req, res) => {
  const { id, title, sector, ticketId, description, responsible, checklist, createdAt } = req.body;
  const connection = await pool.getConnection();
  ...
  try {
    ...
    // Check existance
    const [existing] = await connection.query('SELECT id FROM tasks WHERE id = ?', [id]);
    ...
    if (existing.length > 0) {
       ...
       await connection.query('DELETE FROM task_checklist_items WHERE task_id = ?', [id]);
    } else {
       await connection.query(
...
app.get('/api/rounds', async (req, res) => {
  try {
    // Buscar Logs
    const [rows] = await pool.query('SELECT * FROM round_logs ORDER BY start_time DESC');
    
    // Mapear resultados
    const logs = await Promise.all(rows.map(async (row) => {
       // Buscar fotos
       const [photos] = await pool.query('SELECT photo_base64 FROM round_evidence_photos WHERE round_id = ?', [row.id]);
...
app.get('/api/settings', async (req, res) => {
  try {
    ...
    if (rows.length > 0) {
      ...
      res.json({ companyName: 'RondaGuard', headerColor: '#3b82f6', logo: null });
    }
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
